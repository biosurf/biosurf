...
Overall, this looks quite good. Also, it is worth noting that the distributions of CD3 are not completely identical, despite it being included in all five panels, the distributions are not completely identical, so it would be a hard requirement to impose on imputations. Now, we will use the Earth Mover's Distance (EMD) to look at the imputations more quantitatively.
...
Testing CyTOFmerge performance on the dataset.
...
The EMD ranges for CyTOFmerge are very similar to those of the other two tools - i.e. all tools seem to be most challenged with markers such as CXCR4 and MIP1b.


I don't know if we want to include CyTOFmerge here - it may be good for illustration - but maybe it's not fair?




A final comparison is the runtime. Here we do a small comparison of cyCombine, CytoBackBone, and CyTOFmerge on a machine with 40 cores and 100 gb memory:
...
Let us consider some of the healthy donor PBMC samples from Ogishi CyTOF data. We will use nine samples - four from batch 1 (HC01-HC05) and four from batch 6 (HC14, 16, 20, 24-33). This comprises 3,969,544 cells.
...
To get an idea about the extent of batch effects and to survey the variance of each marker, we will make a density plot for the corrected and uncorrected dataset.
...
Now, let us look at what really matters - the global structure of the data. We will evaluate this using a calculation of the inter-cell distances (in Euclidean space) and comparing the distribution of these.

```{r cell-distances}
# Density of cell-cell distances for 25,000 randomly selected cells (matrix too large for all cells)
...
# Calculations
dist_bc <- ogishi_bc %>% dplyr::filter(id %in% indeces) %>% dplyr::select(all_of(og_markers)) %>% Rfast::Dist()
dist_easy <- ogishi_easy %>% dplyr::filter(id %in% indeces) %>% dplyr::select(all_of(og_markers)) %>% Rfast::Dist()
dist_hard <- ogishi_hard %>% dplyr::filter(id %in% indeces) %>% dplyr::select(all_of(og_markers)) %>% Rfast::Dist()

# Plotting the densities of the distances
plot(density(as.vector(dist_bc[upper.tri(dist_bc)])), col = 'red', main = 'Cell-cell distances')
lines(density(as.vector(dist_easy[upper.tri(dist_easy)])), col = 'blue')
lines(density(as.vector(dist_hard[upper.tri(dist_hard)])), col = 'green')
legend('topright', legend = c('Batch correction', 'Easy-to-impute', 'Hard-to-impute'), col = c('red', 'blue', 'green'), lty = 1)


bins <- seq(-0.1, 30.5, by = 0.1)
distr_bc <- matrix(graphics::hist(as.vector(dist_bc[upper.tri(dist_bc)]), breaks = bins, plot = FALSE)$density)
distr_easy <- matrix(graphics::hist(as.vector(dist_easy[upper.tri(dist_easy)]), breaks = bins, plot = FALSE)$density)
distr_hard <- matrix(graphics::hist(as.vector(dist_hard[upper.tri(dist_hard)]), breaks = bins, plot = FALSE)$density)

emdist::emd2d(distr_bc, 
            distr_easy)
...
Perhaps not a good approach - at least the EMD is more different for the batch corrected vs. the easy than the batch corrected vs. the hard...
...
```{r Ogishi pairwise co-variance}
plot(density(cor(ogishi[,og_markers], method = 'pearson')), ylim = c(0,2.2), main = 'Pairwise Pearson correlation of markers')
lines(density(cor(ogishi_bc[,og_markers], method = 'pearson')), col = 'red')
lines(density(cor(ogishi_easy[,og_markers], method = 'pearson')), col = 'blue')
lines(density(cor(ogishi_hard[,og_markers], method = 'pearson')), col = 'green')
legend('topright', legend = c('Original', 'Batch correction', 'Easy-to-impute', 'Hard-to-impute'), col = c('black', 'red', 'blue', 'green'), lty = 1)
# Global - not necessarily correct...
```


We could now determine the cluster abundance (per sample) - but that requires some labeling and/or cluster matching. It can be quite time-consuming...

Maybe there is just not variance in the data?
...
Write something about the fairly well-preserved structure in easy.

Thoughts: Fewer overlapping markers - less information during batch correction and clustering prior to imputation. This means that the bandwidth of the markers within the clusters used for imputation become larger - which reflects the larger degree of uncertainty associated with a smaller overlap for imputation?




I cannot just mark a set of cells (one from each som, because as we know, they may jump while another jumps 'back').
It is not very directly comparable when simply doing a som-grid... we sort of have to annotate them or something. But then we probably need more clusters? But if we want to compare percentages we need to do it...



"Kan han mene at ved at korrigere på færre markører (kun de overlappende) får man en mindre informeret korrigering end hvis hvert datasæt var korrigeret hver for sig (så bliver de self ikke integreret) og derfor kan noget af den biologiske information blive udvandet (tabt), hvilket videreføres til imputeringen?"


Something about what imputing from and to - from healthy to sick - maybe not a good idea.




Det virker jo hvis cellerne er ens - men det kan ødelægges hvis der er meget forskellige distributioner af celletyper, men man ikke har lineage markører i sit overlap. Så clustrer man bare på den del der ikke er informativ, og så kan man ende med at have underlige clusterfordelinger, der maskerer signalet.
