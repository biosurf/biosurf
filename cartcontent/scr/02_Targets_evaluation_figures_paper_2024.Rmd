...
EGFR_deeploc <- EGFR_deeploc %>% select(-Localizations, -Signals)
...
ensg <- EGFR_transcripts %>% select(`Transcript ID`, `UniProt Match`) %>% 
...
EGFR_heatmap <- EGFR_deeploc %>% column_to_rownames("transcript") %>% select(-`UniProt Match`)

pheatmap(EGFR_heatmap,
...
# remove non-relevant columns 
CD7_deeploc <- CD7_deeploc %>% select(-Localizations, -Signals)
...
ensg <- CD7_transcripts %>% select(`Transcript ID`, `UniProt Match`) %>% 
  separate(., col = `Transcript ID`, into = c("transcript", NA), sep = "\\.")
...
CD7_heatmap <- CD7_deeploc %>% column_to_rownames("transcript") %>% select(-`UniProt Match`)

pheatmap(CD7_heatmap,
         cluster_cols = F, cluster_rows = F)
...
# remove non-relevant columns 
MSLN_deeploc <- MSLN_deeploc %>% select(-Localizations, -Signals)
...
ensg <- MSLN_transcripts %>% select(`Transcript ID`, `UniProt Match`) %>% 
  separate(., col = `Transcript ID`, into = c("transcript", NA), sep = "\\.")

MSLN_deeploc <- ensg %>% right_join(., MSLN_deeploc, by = c("UniProt Match" = "Protein_ID")) %>% 
  drop_na()

MSLN_heatmap <- MSLN_deeploc %>% column_to_rownames("transcript") %>% select(-`UniProt Match`)

pheatmap(MSLN_heatmap,
         cluster_cols = F, cluster_rows = F)
...
# remove non-relevant columns 
CD19_deeploc <- CD19_deeploc %>% select(-Localizations, -Signals)
...
ensg <- CD19_transcripts %>% select(`Transcript ID`, `UniProt Match`) %>% 
  separate(., col = `Transcript ID`, into = c("transcript", NA), sep = "\\.")

CD19_deeploc <- ensg %>% right_join(., CD19_deeploc, by = c("UniProt Match" = "Protein_ID")) 

CD19_heatmap <- CD19_deeploc %>% column_to_rownames("transcript") %>% select(-`UniProt Match`)

pheatmap(CD19_heatmap,
         cluster_cols = F, cluster_rows = F)
