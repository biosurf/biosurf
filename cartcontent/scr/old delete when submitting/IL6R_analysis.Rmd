...
Download protein sequences for all IL6R isoforms from Ensembl (GRCh38, release 115). We retrieve sequences for all protein-coding transcripts to ensure comprehensive coverage.
...
il6r_transcripts <- get_gene_transcripts("IL6R", ensembl)
...
il6r_sequences_all <- get_protein_sequences(il6r_transcripts$ensembl_transcript_id, ensembl)
writeXStringSet(il6r_sequences_all, filepath = "../results/IL6R analysis/il6r_sequences.fasta", format = "fasta")
print(il6r_sequences_all)
```
...
writeXStringSet(il6r_sequences_all, filepath = temp_input, format = "fasta")
...
il6r_aligned <- readAAStringSet(temp_output)
unlink(c(temp_input, temp_output))

cat("Alignment width:", unique(width(il6r_aligned)), "positions\n")
writeXStringSet(il6r_aligned, "../results/IL6R analysis/il6r_aligned_mafft.fasta")
```
...
Find epitopes 
```{r}
# 1. Extract the canonical sequence from your AAStringSet
canonical_id <- "ENST00000368485"
canonical_seq <- il6r_sequences_all[[canonical_id]]

# 2. Find the exact matches for your sequences
match_1 <- matchPattern("AERSKT", canonical_seq)
match_2 <- matchPattern("SSFY", canonical_seq)

# 3. Convert the results into standard data frames to save the positions
pos_AERSKT <- as.data.frame(ranges(match_1))
pos_SSFY <- as.data.frame(ranges(match_2))

# Add a column to identify the motif (helpful if you combine them later)
pos_AERSKT$motif <- "AERSKT"
pos_SSFY$motif <- "SSFY"

# 4. Combine them into one final table of positions
motif_positions <- rbind(pos_AERSKT, pos_SSFY)
...
deeploc_results <- read_csv("../results/IL6R analysis/deeploc21_results.csv", 
...
deeptmhmm_results <- parse_deeptmhmm(
  "../results/IL6R analysis/deeptmhmm10_results.3line"
)
...
aligned_segments <- map_topology_to_alignment(deeptmhmm_results, il6r_aligned)

# ===== Map Epitopes Across Isoforms =====
epitope_positions <- list(
  T = list(c(259, 264), c(185, 188), c(246, 249))
)
...
epitope_mapping <- map_epitopes_from_alignment(
  aligned_seqs = il6r_aligned,
  canonical_name = "ENST00000368485",
...
transcript_order <- c(
  ...
  "ENST00000476006",  
  "ENST00000344086",  
  "ENST00000368485"   # Canonical
)

transcript_order <- c("ENST00000515190", "ENST00000368485")
...
# Move this BEFORE the deeploc_heatmap ggplot call
plot_data_combined <- deeploc_filtered %>%
...
deeploc_heatmap <- ggplot(plot_data_combined,
  ...
  labs(title = "IL6R Isoforms", x = NULL, y = NULL, fill = "Probability") +
  theme_minimal() +
  theme(
...
aligned_segments_sub <- aligned_segments %>%
  filter(transcript_id %in% transcript_order)

epitope_mapping_sub <- epitope_mapping %>%
  filter(transcript_id %in% transcript_order)

topology_plot <- plot_topology_with_epitopes(aligned_segments_sub, epitope_mapping_sub, transcript_order)
...
# ggsave("../results/IL6R analysis/il6r_localization_topology_epitopes.pdf",
#        combined_plot, width = 20, height = 8)
ggsave("../results/IL6R analysis/il6r_localization_topology_epitopes_only2.pdf",
       combined_plot, width = 15, height = 6)
```
