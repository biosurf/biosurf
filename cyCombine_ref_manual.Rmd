...
# Evaluate performance using Earth Mover's Distance (EMD) reduction

The EMD reduction is implemented as a performance metric; EMDs are computed for both the uncorrected and corrected data, removing those values where both had an EMD < 2.
...
# Evaluate performance using Median Absolute Deviation (MAD) score

The MAD score is implemented as another performance metric; MADs are computed for both the uncorrected and corrected data per-cluster, per-marker, per-batch. The MAD score is then calculated as the median of the absolute difference in MAD per value:
